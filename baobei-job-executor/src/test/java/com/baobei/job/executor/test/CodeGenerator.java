package com.baobei.job.executor.test;

import com.baobei.job.executor.constants.ProjectConstant;
...
/**
 * 代码生成器，根据数据表名称生成对应的Domain、Mapper、Service、ServiceImpl、Controller、JobHandler、DTO、XML简化开发。
 *
...
public class CodeGenerator {
    ...
    /** 生成的JobHandler存放路径 */
    private static final String PACKAGE_PATH_JOBHANDLER = packageConvertPath(ProjectConstant.JOBHANDLER_PACKAGE);
    /** 生成的DTO存放路径 */
...
//        genController(tableName, domainName, description, pkDataType);
        genJobhandler(tableName, domainName, description);
    ...
    public static void genJobhandler(String tableName, String domainName, String description) {
        try {
            freemarker.template.Configuration cfg = getConfiguration();
            ...
            data.put("author", AUTHOR);
            String domainNameUpperCamel = StringUtils.isEmpty(domainName) ? tableNameConvertUpperCamel(tableName) : domainName;
            data.put("baseRequestMapping", tableNameConvertLowerCamel(tableName)+"JobHandler");
            data.put("domainNameUpperCamel", domainNameUpperCamel);
            data.put("domainNameLowerCamel", tableNameConvertLowerCamel(tableName));
            data.put("basePackage", ProjectConstant.BASE_PACKAGE);
            data.put("servicePackage", ProjectConstant.SERVICE_PACKAGE);

            File file = new File(PROJECT_PATH + JAVA_PATH + PACKAGE_PATH_JOBHANDLER + domainNameUpperCamel + "JobHandler.java");
            if (!file.getParentFile().exists()) {
                file.getParentFile().mkdirs();
            }
            cfg.getTemplate("jobhandler.ftl").process(data, new FileWriter(file));
            System.out.println(domainNameUpperCamel + "JobHandler.java 生成成功");
        } catch (Exception e) {
            throw new RuntimeException("生成JobHandler失败", e);
        }
